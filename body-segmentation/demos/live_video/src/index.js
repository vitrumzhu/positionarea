...
async function app() {
  ...
  setInterval(function() {
    ...
    let areafactor = (1 - blackPixelCount / (canvas.width * canvas.height)) * 100;
    document.getElementById('positionarea').innerText = 'Area Factor: ' + areafactor.toFixed(2);
  }, 500);
  ...
  document.getElementById('screenshotButton').addEventListener('click', function() {
    ...
    setTimeout(function() {
      recorder.stopRecording(function() {
        ...
        var dateString = currentDate.toISOString().slice(0, 19).replace(/[-T:/]/g, '');
        var randomString = Math.random().toString(36).substring(2, 10);
        var fileName = dateString + '_' + randomString + '.png';
        a.download = fileName;
